...
lora0_sigfox1 = 1       # 0 to choose LoRa, 1 to choose Sigfox
...
if (lora0_sigfox1 == 0):
    ...
    lorawan_socket.setsockopt(socket.SOL_LORA, socket.SO_DR, 5)
    ...
    lorawan_socket = socket.socket(socket.AF_SIGFOX, socket.SOCK_RAW)

    # make the socket blocking
    lorawan_socket.setblocking(True)

    # configure it as uplink only
    lorawan_socket.setsockopt(socket.SOL_SIGFOX, socket.SO_RX, False)
    ...
    while True:
        ...
        # Data analysis
            ...
            if (decoded_message[0] == "ID"):
                ...
                print(temp_bytes)
                lorawan_socket.send(bytes[temp_bytes])
            lorawan_socket.setblocking(False)
            
            # get any data received (if any...)
            #data = lorawan_socket.recv(64)
            #print(data)
